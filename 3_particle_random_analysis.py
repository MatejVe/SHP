...
def convert_to_timestep(filepath):
    ...
    for line in f.readlines():
        ...
        for i in range(int(time // ts)):
            for j in range(3):
                newPos = (
                    poss[j] + (i + 1) * ts * vels[j] + (ts - leftTime) * vels[j]
                ) % 1
                newPos = newPos + D("1") if newPos < 0 else newPos
                positions[j].append(float(newPos))
...
for i in range(10):
    masses, positions, velocities = convert_to_timestep(
        "Experiments/3_particles/random" + str(i)
    )
    ...
    axes[0].hist(
        x=positions[2], color="green", bins=1000, histtype="step", density=True
    )
    ...
    axes[1].hist(
        x=velocities[0], color="blue", bins=1000, histtype="step", density=True
    )
    axes[1].hist(x=velocities[1], color="red", bins=1000, histtype="step", density=True)
    axes[1].hist(
        x=velocities[2], color="green", bins=1000, histtype="step", density=True
    )
