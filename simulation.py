...
def n3_check_for_undetected_collision(
    collisionIndices,
):  # n3 stands for this only works for three particles
...
class Sistem:
    ...
    def __init__(
        ...
        particleNum,
        masses="random",
        initPositions="random",
        initVelocities="random",
    ):
        ...
        if masses == "random":
            masses = (
                np.random.random(size=particleNum) + 0.00001
            )  # Adding a small constant to avoid assigning a mass of 0
        elif not isinstance(masses, list) and not isinstance(masses, np.ndarray):
            raise Exception(
                'Not a valid input for the masses, if not "random" the input has to be a list of masses!'
            )
        elif len(masses) != particleNum:
            raise Exception(
                "Differing amount of masses to the amount of particles was provided!"
            )

        if initPositions == "random":
            initPositions = np.sort(
                np.random.random(size=particleNum)
            )  # Sort the array so that the nearest neighbors of a particle i are always particles (i+1) and (i-1)
        elif not isinstance(initPositions, list) and not isinstance(
            initPositions, np.ndarray
        ):
            raise Exception(
                'Not a valid input for the masses, if no "random" the input has to be a list of positions!'
            )
        elif len(initPositions) != particleNum:
            raise Exception(
                "Differing amount of positions to the amount of particles was provided!"
            )

        if initVelocities == "random":
            initVelocities = (
                2 * np.random.random(size=particleNum) - 1
            )  # Initialize random velocities in the range [-1, 1)
        elif not isinstance(initVelocities, list) and not isinstance(
            initVelocities, np.ndarray
        ):
            raise Exception(
                'Not a valid input for the velocities, if not "random" the input has to be a list of velocities!'
            )
        elif len(initVelocities) != particleNum:
            raise Exception(
                "Differing amount of velocities to the amount of particles was provided!"
            )

        self.particles = [
            Particle(mass, initPos, initVel, index)
            for index, (mass, initPos, initVel) in enumerate(
                zip(masses, initPositions, initVelocities)
            )
        ]
        ...
        self.energy = sum(
            [
                Decimal("0.5") * particle.mass * particle.velocity**2
    ...
    def find_next_event_s(self):
        ...
        for i in range(self.particleNum):
            if can_collide(self.particles[i - 1], self.particles[i]):
                distanceLeft = ring_distance(
                    self.particles[i - 1], self.particles[i]
                )  # Only check with the particle to the left, to avoid doublechecking

                if (
                    self.particles[i - 1].velocity == self.particles[i].velocity
                ):  # if the particles have the same velocity they will never collide
                    time = 1e10
                else:
                    time = distanceLeft / (
                        abs(self.particles[i - 1].velocity - self.particles[i].velocity)
                    )
        ...
        for _ in range(self.collisionNumber):
            ...
            if shouldPrint is not None:
                for (
                    ...
                    shouldPrint
                ):  # Just fancy pritting tools, not important to the physics
                    if hasattr(self.sistem, attribute):
                        values = getattr(self.sistem, attribute)
                        if isinstance(values, list):
                            print(attribute + ":", end=" ")
                            ...
                            print(
                                f"{attribute}:{getattr(self.sistem, attribute):.3f} |",
                                end=" ",
                            )
